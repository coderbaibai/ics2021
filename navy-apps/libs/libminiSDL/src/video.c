...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h) {
  ...
  if(s->format->BytesPerPixel==4)
    ...
    for(int i=0;i<w*h;i++){
      *(cur_pixels+i) = ((uint32_t)s->format->palette->colors[*(s->pixels+i)].a<<24)|((uint32_t)s->format->palette->colors[*(s->pixels+i)].r<<16)|((uint32_t)s->format->palette->colors[*(s->pixels+i)].g<<8)|((uint32_t)s->format->palette->colors[*(s->pixels+i)].b);
